...
class EXE extends Module with HasCoreParameter {
  val io = IO(new Bundle {
    val alu     = Flipped(new ALUOPBundle)
  ...
  alu_.io.alu_op := io.alu.calc
  alu_.io.op1_v := Mux1H(
    Seq(
      (io.alu.op1_sel === ALU_op1_sel.alu_op1sel_ZERO) -> 0.U,
      (io.alu.op1_sel === ALU_op1_sel.alu_op1sel_RS1)  -> io.rf.read.rs1_v,
      (io.alu.op1_sel === ALU_op1_sel.alu_op1sel_PC)   -> (io.pc)
    )
  )
  alu_.io.op2_v := Mux1H(
    Seq(
      (io.alu.op2_sel === ALU_op2_sel.alu_op2sel_ZERO) -> 0.U,
      (io.alu.op2_sel === ALU_op2_sel.alu_op2sel_IMM)  -> io.imm,
      (io.alu.op2_sel === ALU_op2_sel.alu_op2sel_RS2)  -> io.rf.read.rs2_v
    )
  )
  ...
  bru_.io.in.rs1_v  := io.rf.read.rs1_v
  bru_.io.in.rs2_v  := io.rf.read.rs2_v
  ...
  io.if_.imm   := io.imm //
  io.if_.rs1_v := io.rf.read.rs1_v // 只有 jalr 会用
