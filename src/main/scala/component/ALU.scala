...
class ALU extends Module with HasCoreParameter {
  val io = IO(new Bundle {
    ...
    val op2    = Input(UInt(XLEN.W)) // 这个可能是: 立即数, 寄存器值
  ...
  switch(io.alu_op) {
    ...
    is(ALUOpType.alu_ADD) {
      io.out := (io.op1.asSInt + io.op2.asSInt).asUInt
    }
    ...
    is(ALUOpType.alu_SLL) {
      io.out := io.op1 << io.op2(log2Up(XLEN) - 1, 0) // 假设最多移位 31 位
    }
    is(ALUOpType.alu_SRL) {
      io.out := io.op1 >> io.op2(log2Up(XLEN) - 1, 0)
    }
    is(ALUOpType.alu_SRA) {
      io.out := (io.op1.asSInt >> io.op2(log2Up(XLEN) - 1, 0)).asUInt
    }
    ...
    is(ALUOpType.alu_JALR) {
      // rs1
      // rs2 = sext(offset)
      io.out := (io.op1 + io.op2) & ~1.U(XLEN.W)
    }
  }
}
