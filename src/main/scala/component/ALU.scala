...
object ALUOpType extends ChiselEnum {
  val alu_X, alu_ADD, alu_SUB, alu_AND, alu_OR, alu_XOR, alu_SLL, alu_SRL, alu_SRA, alu_SLT, alu_SLTU, alu_BR, alu_JAL, alu_JALR = Value
}

class ALU extends Module with HasCoreParameter {
  val io = IO(new Bundle {
    val rs1    = Input(UInt(XLEN.W))
    val rs2    = Input(UInt(XLEN.W)) // 这个可能是: 立即数, 寄存器值, 或者是 PC
